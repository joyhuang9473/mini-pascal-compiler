...
PROCFUNCDEF *make_procfuncdef (DECLLIST *formals, IDENT *type_id, BLOCK *b)
{
  PROCFUNCDEF *p = anew (PROCFUNCDEF);
  p->proc_formals = formals;
  p->proc_result_type = ( type_id != NULL ) ? make_id_type(type_id) : NULL;
  p->proc_block = b;
  return p;
}
...
DECL *make_procfunc_decl (IDENT *name_id, DECLLIST *formals, IDENT *type_id, BLOCK *b)
{
  DECL *d = make_decl (DeclProcFunc_, name_id);
  d->d.proc = make_procfuncdef (formals, type_id, b);
  return d;
}
...
TYPE *make_record_fieldList_type (FIELDLIST *fieldList)
{
...
FIELD *make_field (IDENT *id, TYPE *type)
{
  FIELD *f = anew (FIELD);
...
FIELDLIST *make_fieldlist (FIELD *field, FIELDLIST *opt_fieldlist)
{
  FIELDLIST *fl = anew (FIELDLIST);

  fl->this = field;
  fl->rest = opt_fieldlist;
  return fl;
}
...
STMTIF *make_stmtif (EXPR *expr, STMT *then, STMT *optElse)
{
...
STMTREPEAT *make_stmtrepeat (EXPR *expr, STMTLIST *stmts)
{
...
/* CASE LIST helper constructors */

CASEARM *make_caselist_element (EXPRLIST *labels, STMT *stmt)
{
  CASEARM *e = anew(CASEARM);
  e->arm_labels = labels;
  e->arm_stmt = stmt;
  return e;
}

CASELIST *make_caselist_elements (CASEARM *element, CASELIST *elements)
{
  CASELIST *e = anew(CASELIST);
  e->this = element;
  e->rest = elements;
  return e;
}
...
STMT *make_compound_stmt (STMTLIST *stmts)
{
...
STMT *make_case_stmt (EXPR *expr, CASELIST *list)
{
  STMT *s = make_stmt (StmtCase_);
  s->s.cas = make_stmtcase (expr, list);
  return s;
}

STMT *make_if_stmt (EXPR *expr, STMT *then, STMT *optElse)
{
  STMT *s = make_stmt (StmtIf_);
  s->s.ifx = make_stmtif (expr, then, optElse);
  return s;
}

STMT *make_repeat_stmt (EXPR *expr, STMTLIST *stmts)
{
  STMT *s = make_stmt (StmtRepeat_);
  s->s.rpt = make_stmtrepeat (expr, stmts);
  return s;
}
